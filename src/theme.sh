...
### Load Options
border_style_active_pane=$(get_tmux_option "@theme_active_pane_border_style" "$(get_palette_color dark5)")
border_style_inactive_pane=$(get_tmux_option "@theme_inactive_pane_border_style" "$(get_palette_color bg_highlight)")
...
# message styling
tmux set-option -g message-style "bg=$(get_palette_color red),fg=$(get_palette_color bg_dark)"

# status bar
status_bar_bg=$(get_palette_color bg_highlight)
if [ "$transparent" = "true" ]; then
	status_bar_bg="default"
fi
tmux set-option -g status-style "bg=${status_bar_bg},fg=$(get_palette_color white)"
...
if [ "$theme_disable_plugins" -ne 1 ]; then
	...
	for plugin in "${plugins[@]}"; do
		...
		if [ ! -f "${CURRENT_DIR}/plugin/${plugin}.sh" ]; then
			...
			plugin_icon=$(eval echo \$${icon_var})
			accent_color=$(eval echo \$${accent_color_var})
			accent_color_icon=$(eval echo \$${accent_color_icon_var})
			...
			if [ "$plugin" != "battery" ]; then
				accent_color="$(get_palette_color "$accent_color")"
				accent_color_icon="$(get_palette_color "$accent_color_icon")"
			fi

			separator_end="#[fg=$(get_palette_color bg_highlight),bg=${accent_color}]${right_separator}#[none]"
			separator_icon_start="#[fg=${accent_color_icon},bg=$(get_palette_color bg_highlight)]${right_separator}#[none]"
			...
			if [ "$plugin" == "datetime" ] || [ "$plugin" == "battery" ]; then
				plugin_output="#[fg=$(get_palette_color white),bg=${accent_color}]${plugin_execution_string}#[none]"
			else
				plugin_output="#[fg=$(get_palette_color white),bg=${accent_color}]#($plugin_script_path)#[none]"
			fi

			plugin_icon_output="${separator_icon_start}#[fg=$(get_palette_color white),bg=${accent_color_icon}]${plugin_icon}${separator_icon_end}"
